...
final class AS7341Tests: XCTestCase {
    ...
    func testIsDataReady() {
        i2c.written = []
        i2c.expectRead = [64]

        XCTAssertEqual(as7341.isDataReady(), true)
    ...
    func testEnableSmux() {
        ...
        as7341.enableSmux()
        XCTAssertEqual(i2c.written, [0x80, 0x80, 19])
    }

    func testsetSmuxCommand() {
        i2c.written = []
        as7341.setSmuxCommand(2)
        XCTAssertEqual(i2c.written, [0xAF, 2])
    }

    func testSetF1F4() {
        i2c.written = []
        i2c.expectRead = [67, 3, 65, 64]
